...
const getActiveRouteState = function (
  ...
  if (!route.state && (!route.routes || route.routes.length === 0)) {
    const link =
      (linking as PathConfig<Record<string, unknown>>)?.screens?.[route.name] ??
      linking;
  ...
  const childActiveRoute = nav?.routes[nav.index] as unknown as
    | Route
    | undefined;
...
export const ActiveRouteProvider = ({
  ...
  return (
    <ActiveRouteContext.Provider value={{routeName: activeRoute}}>
      {children}
...
export const useActiveRoute = () => {
  ...
  const {routeName} = useContext(ActiveRouteContext);
